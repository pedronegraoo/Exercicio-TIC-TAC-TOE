...
//serve para quando o jogador clicar no campo selecionado
function handleBoardClick(evento) {
  const span = evento.currentTarget;
  const region = evento.currentTarget.dataset.region; //N.N
  //método split serve para dividir uma string transformando ela em um array
  ...
  disableRegion(span);
  //Código para verificar se o jogador venceu
...
document.getElementById("start").addEventListener("click", preparacao);
